...
module RefugeePayments
  ...
  module ClassMethods
    ...
    def amount_and_days(pms, report_range = default_date_range)
      pms.map do |pm|
        period_length = (pm.period_end - pm.period_start).to_i
        daily_amount = period_length.zero? ? 0 : pm.amount / period_length
        days = days_for_payment(pm, report_range)
